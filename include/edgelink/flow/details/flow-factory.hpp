...
class FlowFactory : public edgelink::IFlowFactory {
  public:
    ...
    std::vector<std::unique_ptr<IFlow>> create_flows(const boost::json::array& flows_config) const override;

    std::vector<std::unique_ptr<edgelink::IStandaloneNode>>
    create_global_nodes(const boost::json::array& flows_config) const override;
