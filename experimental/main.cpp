...
using Endpoint = am::endpoint<async_mqtt::role::client, async_mqtt::protocol::mqtt>;

template <typename Executor> as::awaitable<void> publish(Executor exe, Endpoint* amep, const std::string_view topic) {

    auto topic_buffer = am::allocate_buffer(topic);
    auto payload_buffer = am::allocate_buffer("payload1");
    auto pid = *amep->acquire_unique_packet_id();
    // Send MQTT PUBLISH
    if (auto se = co_await amep->send(
            am::v3_1_1::publish_packet{pid, topic_buffer, payload_buffer, am::qos::at_least_once}, as::use_awaitable)) {
    ...
    for (std::size_t count = 0; count != 2; ++count) {
        if (am::packet_variant pv = co_await amep->recv(as::use_awaitable)) {
            pv.visit(am::overload{[&](am::v3_1_1::publish_packet const& p) {
            ...
            std::cout << "MQTT recv error:" << pv.get<am::system_error>().what() << std::endl;
            co_return;
        }
    }
}
...
template <typename Executor> as::awaitable<void> subscribe(Executor exe, Endpoint* amep) {
    ...
    // Recv MQTT SUBACK
    if (am::packet_variant pv = co_await amep->recv(as::use_awaitable)) {
        auto cb = am::overload{[&](am::v3_1_1::suback_packet const& p) {
...
as::awaitable<void> proc(Executor exe, const std::string_view host, const std::string_view port) {
    ...
    try {
        ...
        co_await as::async_connect(amep.next_layer(), eps, as::use_awaitable);
        ...
        // as::co_spawn(exe, subscribe(exe, &amep), as::detached);
        co_await subscribe(exe, &amep);

        std::cout << "开始发布五次" << std::endl;
        for (size_t i = 0; i < 5; i++) {
            co_await publish(exe, &amep, "topic1");
        }
